"""
test_gis.py — testy dla modułu ishkarim_gis.
"""
import pytest
...
class TestImport:
    ...
    def test_import_main(self):
        import ishkarim_gis
        assert ishkarim_gis.__version__ == "0.1.0"

    def test_area(self):
        import ishkarim_gis
        assert ishkarim_gis.__area__ == "gis"

    def test_modules_list_nonempty(self):
        import ishkarim_gis
        assert len(ishkarim_gis.MODULES) > 0

    def test_submodule_imports(self):
        from ishkarim_gis import utils
...
class TestKnowledgeIndex:
    ...
    def test_returns_list(self, tmp_path):
        import ishkarim_gis
        result = ishkarim_gis.load_knowledge_index(root=str(tmp_path))
        assert isinstance(result, list)

    def test_record_keys(self, tmp_path):
        import os, ishkarim_gis
        # create a minimal fake source dir
        src_dir = tmp_path / ishkarim_gis.MODULES[0]
...
class TestUtils:
    ...
    def test_read_work_md_missing(self, tmp_path):
        from ishkarim_gis.utils import read_work_md
    ...
    def test_extract_python_blocks(self):
        from ishkarim_gis.utils import extract_python_blocks
        md = """```python
def greet():
    return "hello world"
