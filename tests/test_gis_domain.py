"""
test_gis_domain.py — testy domenowe dla ishkarim_gis.
...
import ishkarim_gis
from ishkarim_gis import load_knowledge_index


@pytest.fixture
def repo_root():
    """Zwraca ścieżkę do katalogu głównego repozytorium Ishkarim."""
    root = Path(__file__).resolve().parents[3]  # projects/ishkarim-gis/tests/ → repo root
...
class TestModuleIntegrity:
    ...
    def test_version_format(self):
        assert re.match(r"\d+\.\d+\.\d+", ishkarim_gis.__version__)

    def test_modules_all_strings(self):
        for name in ishkarim_gis.MODULES:
