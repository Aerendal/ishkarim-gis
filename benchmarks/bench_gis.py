#!/usr/bin/env python3
"""
bench_gis.py — benchmarki wydajnościowe dla ishkarim_gis.

Mierzy:
  1. Czas importu modułu
  2. Czas load_knowledge_index()
  3. Przepustowość extract_python_blocks()
  4. Testy specyficzne dla obszaru gis

Uruchomienie:
    python projects/ishkarim-gis/benchmarks/bench_gis.py
    python projects/ishkarim-gis/benchmarks/bench_gis.py --quick
"""
...
def bench_load_index(root: str, n: int = 3) -> tuple[float, int]:
    ...
    for _ in range(n):
        t0 = time.perf_counter()
        docs = ishkarim_gis.load_knowledge_index(root=root)
        times.append(time.perf_counter() - t0)
        count = len(docs)
    times.sort()
...
def bench_extract_blocks(root: str, n_files: int = 20) -> tuple[float, int]:
    ...
    from ishkarim_gis.utils import read_work_md, extract_python_blocks
    import ishkarim_gis
    docs = ishkarim_gis.load_knowledge_index(root=root)
    sample = docs[:n_files]
...
def bench_tags_parse(root: str, n_files: int = 50) -> tuple[float, int]:
    ...
    from ishkarim_gis.utils import extract_tags
    import ishkarim_gis
    docs = ishkarim_gis.load_knowledge_index(root=root)
    sample = docs[:n_files]
...
def run_area_benchmarks(root: str, quick: bool) -> list[tuple[str, str, str]]:
    ...
    import importlib
    mod = importlib.import_module("ishkarim_gis")
    docs = mod.load_knowledge_index(root=root)
    n = min(len(docs), 20 if not quick else 5)

    from ishkarim_gis.utils import extract_tags, read_work_md
    t0 = time.perf_counter()
    for doc in docs[:n]:
...
def main():
    parser = argparse.ArgumentParser(description="Benchmarki dla ishkarim-gis")
    ...
    print(f"\n============================================================")
    print(f"  Benchmark: ishkarim-gis")
    print(f"  Root: {root}")
